...
LaserWebServer::LaserWebServer() : server(80) {
  ...
  for (int i = 0; i < 4; i++) {
    for (int j = 0; j < 48; j++) {
      deviceStates[i][j] = 0;
      shieldMask[i][j] = 0; // 初始化所有屏蔽位为0 (不屏蔽)
    }
...
String LaserWebServer::getHTMLPage() {
  ...
  html += "        }\n";
  html += "        .state-shielded {\n";
  html += "            background-color: #ffc107 !important;\n";
  html += "            border-color: #d39e00 !important;\n";
  html += "        }\n";
  html += "        .status-bar {\n";
  html += "            text-align: center;\n";
  html += "            padding: 10px;\n";
  ...
  html += "            <input type=\"number\" id=\"baselineDelay\" min=\"0\" "
          "max=\"5000\" step=\"50\" value=\"200\">\n";
  html +=
      "            <button onclick=\"setBaselineDelay()\">Set Delay</button>\n";
  html += "            <button onclick=\"getCurrentBaselineDelay()\">Get "
          "Current</button>\n";
  html += "        </div>\n";
  html += "        \n";
  ...
  html += "            <span id=\"connectionStatus\" class=\"connection-status "
  ...
  html += "            <!-- Device cards will be dynamically generated by "
          "JavaScript -->\n";
  html += "        </div>\n";
  html += "    </div>\n";
  html += "\n";
  ...
  html += "        var shieldMask = {};\n";
  html += "\n";
  html += "        function loadShieldMask() {\n";
  html += "            fetch('/api/shield')\n";
  html += "            .then(response => response.json())\n";
  html += "            .then(data => {\n";
  html += "                shieldMask = data;\n";
  html += "                updateAllShieldDisplay();\n";
  html += "            })\n";
  html += "            .catch(error => console.error('Failed to load shield "
          "mask:', error));\n";
  html += "        }\n";
  html += "\n";
  html += "        function toggleShield(deviceId, inputId) {\n";
  html += "            var deviceKey = 'device' + deviceId;\n";
  html += "            var currentList = shieldMask[deviceKey] || [];\n";
  html += "            var isShielded = currentList.includes(inputId);\n";
  html += "            var newState = !isShielded;\n";
  html += "            \n";
  html += "            fetch('/api/shield', {\n";
  html += "                method: 'POST',\n";
  html += "                headers: { 'Content-Type': 'application/json' },\n";
  html += "                body: JSON.stringify({ device: deviceId, id: "
          "inputId, state: newState })\n";
  html += "            })\n";
  html += "            .then(response => response.json())\n";
  html += "            .then(data => {\n";
  html += "                if(data.status === 'ok') {\n";
  html += "                    if (newState) {\n";
  html += "                        if (!shieldMask[deviceKey]) "
          "shieldMask[deviceKey] = [];\n";
  html += "                        shieldMask[deviceKey].push(inputId);\n";
  html += "                        showStatusMessage('Shielded Device ' + "
          "deviceId + ' Input ' + inputId, 'success');\n";
  html += "                    } else {\n";
  html += "                        if (shieldMask[deviceKey]) {\n";
  html += "                            shieldMask[deviceKey] = "
          "shieldMask[deviceKey].filter(id => id !== inputId);\n";
  html += "                        }\n";
  html += "                        showStatusMessage('Unshielded Device ' + "
          "deviceId + ' Input ' + inputId, 'success');\n";
  html += "                    }\n";
  html += "                    updateInputDisplay(deviceId, inputId);\n";
  html += "                } else {\n";
  html += "                    showStatusMessage('Failed to update shield', "
          "'error');\n";
  html += "                }\n";
  html += "            });\n";
  html += "        }\n";
  html += "\n";
  html += "        function updateAllShieldDisplay() {\n";
  html += "            for (var deviceId = 1; deviceId <= 4; deviceId++) {\n";
  html += "                var deviceKey = 'device' + deviceId;\n";
  html += "                if (shieldMask[deviceKey]) {\n";
  html += "                    shieldMask[deviceKey].forEach(inputId => {\n";
  html += "                        updateInputDisplay(deviceId, inputId);\n";
  html += "                    });\n";
  html += "                }\n";
  html += "            }\n";
  html += "        }\n";
  html += "\n";
  html += "        function updateInputDisplay(deviceId, inputId) {\n";
  html += "            var el = document.getElementById('device-' + deviceId + "
          "'-input-' + inputId);\n";
  html += "            if (!el) return;\n";
  html += "            \n";
  html += "            var deviceKey = 'device' + deviceId;\n";
  html += "            var isShielded = false;\n";
  html += "            if (shieldMask[deviceKey] && "
          "shieldMask[deviceKey].includes(inputId)) {\n";
  html += "                isShielded = true;\n";
  html += "            }\n";
  html += "            \n";
  html += "            var isActive = el.classList.contains('state-active');\n";
  html += "            \n";
  html += "            if (isShielded) {\n";
  html += "                el.className = 'input-state state-shielded';\n";
  html += "            } else if (isActive) {\n";
  html += "                el.className = 'input-state state-active';\n";
  html += "            } else {\n";
  html += "                el.className = 'input-state state-inactive';\n";
  html += "            }\n";
  html += "        }\n";
  html += "\n";
  html += "        function setBaselineDelay() {\n";
  html += "            var delay = "
          "document.getElementById('baselineDelay').value;\n";
  html += "            \n";
  ...
  html += "            .then(response => response.json())\n";
  html += "            .then(data => {\n";
  html += "                showStatusMessage('Baseline delay set to ' + delay "
          "+ 'ms', 'success');\n";
  html += "            })\n";
  html += "            .catch(error => {\n";
  html += "                console.error('Failed to set baseline delay:', "
          "error);\n";
  html += "                showStatusMessage('Failed to set baseline delay', "
          "'error');\n";
  html += "            });\n";
  html += "        }\n";
  html += "\n";
  ...
  html += "            .then(response => response.json())\n";
  html += "            .then(data => {\n";
  html += "                document.getElementById('baselineDelay').value = "
          "data.delay;\n";
  html += "                showStatusMessage('Current baseline delay: ' + "
          "data.delay + 'ms', 'success');\n";
  html += "            })\n";
  html += "            .catch(error => {\n";
  html += "                console.error('Failed to get baseline delay:', "
          "error);\n";
  html += "                showStatusMessage('Failed to get baseline delay', "
          "'error');\n";
  html += "            });\n";
  html += "        }\n";
  html += "\n";
  html += "        function showStatusMessage(message, type) {\n";
  html += "            var statusElement = "
  ...
  html += "                } catch (e) {\n";
  html += "                    console.error('Failed to parse EventSource "
          "message:', e);\n";
  html += "                }\n";
  html += "            };\n";
  html += "            \n";
  ...
  html += "                statusElement.className = 'connection-status "
          "connected';\n";
  html += "            } else {\n";
  ...
  html += "            document.getElementById('lastUpdate').textContent = "
          "timeString;\n";
  html += "        }\n";
  html += "\n";
  ...
  html += "            \n";
  html += "            card.innerHTML = \n";
  html += "                '<div class=\"device-title\">Device ' + deviceId + "
          "'</div>' +\n";
  html += "                '<div class=\"input-grid\" id=\"input-grid-' + "
          "deviceId + '\">' +\n";
  html += "                '<!-- Input status will be dynamically generated by "
          "JavaScript -->' +\n";
  html += "                '</div>';\n";
  html += "            \n";
  html += "            return card;\n";
  ...
  html += "                if (inputs) {\n";
  html += "                    var inputGrid = "
          "document.getElementById('input-grid-' + deviceId);\n";
  html += "                    \n";
  html += "                    // If first load, create all input items\n";
  html += "                    if (inputGrid.children.length === 0) {\n";
  html += "                        for (var inputId = 1; inputId <= 48; "
          "inputId++) {\n";
  html += "                            var inputItem = "
          "document.createElement('div');\n";
  html += "                            inputItem.className = 'input-item';\n";
  html += "                            inputItem.innerHTML = \n";
  html += "                                '<div class=\"input-id\">' + "
          "inputId + '</div>' +\n";
  html += "                                '<div class=\"input-state\" ' +\n";
  html += "                                'id=\"device-' + deviceId + "
          "'-input-' + inputId + '\" ' +\n";
  html += "                                'onclick=\"toggleShield(' + "
          "deviceId + ', ' + inputId + ')\"></div>';\n";
  ...
  html +=
      "                        var stateElement = "
      "document.getElementById('device-' + deviceId + '-input-' + input.id);\n";
  html += "                        if (stateElement) {\n";
  html += "                            if (input.state === 1) {\n";
  html += "                                "
          "stateElement.classList.add('state-active');\n";
  html += "                                "
          "stateElement.classList.remove('state-inactive');\n";
  html += "                            } else {\n";
  html += "                                "
          "stateElement.classList.add('state-inactive');\n";
  html += "                                "
          "stateElement.classList.remove('state-active');\n";
  html += "                            }\n";
  html +=
      "                            updateInputDisplay(deviceId, input.id);\n";
  html += "                        }\n";
  html += "                    }\n";
  html += "                }\n";
  html += "            }\n";
